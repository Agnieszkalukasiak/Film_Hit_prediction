...
def page_predictor_pipeline_body():
    ...
    tabs = st.tabs(["Predictions", "Residuals", "Distribution", "ROI"])
    ...
    # Sample input
        ...
        # Display results
                st.write("Predicted Revenue", f"${final_revenue:,.2f}")
                st.write("Profit/Loss", f"${profit_loss:,.2f}")
                st.write("ROI", f"{roi:.1f}%")
            
        # Model Performance Metrics
                st.header("4. Model Performance Metrics")
                st.write("R² Score", "0.150")
                st.write("MAE", "$0.89")
                st.write("RMSE", "$1.06")
