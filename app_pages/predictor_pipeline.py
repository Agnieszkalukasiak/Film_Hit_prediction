...
def page_pipeline_overview():
    ...
    st.info(
        f"* **The regression** model achieved a **strong R² score of 0.96 on the training set**, indicating that it effectively captures the underlying revenue-driving factors in the data. "
        f"However, the significantly lower **R² score of 0.78 on the test set** suggests that the model may be overfitting, as it struggles to generalize to unseen data. \n\n"
        f"* The training performance, with a **Mean Absolute Error (MAE) of 16.23 million**, "
        f"demonstrates that the model performs exceptionally well on known data. However, the significantly higher **test error (MAE: 43.89 million)** highlights the challenges in predicting revenue accurately for new films, "
